...
void listQueueInit(struct listQueue *q)
{
    ...
    q->firstLink = first;
	struct link * last = malloc(sizeof(struct link));
    q->lastLink = last;
	last->value = 0;
	last->next = NULL;
	first->value = 0;
	first->next = q->lastLink;
}
...
{
    assert(q != NULL);
    struct link*add = malloc(sizeof(struct link));
    add->value = e;
    assert(add != NULL);
...
{
    assert(q != NULL);
    struct link *needMove = q->firstLink->next;
    q->firstLink->next = needMove->next;
    q->size--;
}
...
void linkedListStackInit(struct linkedListStack * s)
{
    assert(s != NULL);
    s->structSize = 0;
	struct listQueue * first = listQueueCreate();
    s->Q1 = first;
	struct listQueue * second = listQueueCreate();
    s->Q2 = second;
 }
...
{
    struct linkedListStack* newLinkedListStack = malloc(sizeof(struct linkedListStack));
	linkedListStackInit(newLinkedListStack);
	return newLinkedListStack;
}
...
void linkedListStackPop(struct linkedListStack *s)
{
    ...
    {
        for(int count = 1; count < s->structSize; count++)
        {
        ...
        listQueueRemoveFront(s->Q1);
        s->structSize--;
        for(int count = 0; count < s->structSize; count++)
        {
